...
def createAccessToken(data: dict, expiresInMinutes:int = 30) -> str:
  ...
  encoded_jwt = jwt.encode(to_encode, config.secret_key, algorithm=config.algoritham or "HS256")
  return encoded_jwt

def decodeAccessToken(token: str) -> dict:
  config = getAppConfig()
  payload = jwt.decode(token, config.secret_key, algorithms=[config.algoritham or "HS256"])
  return payload